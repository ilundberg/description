...
## Determine challenge truth

d_2022 <- read_dta("/Users/ilundberg/Library/CloudStorage/Box-Box/description_project/data/usa_00043.dta") |>
  # Restrict to full-year full-time workers
  filter(wkswork2 == 6 & uhrswork >= 35) |>
  # Inflation adjustment
  left_join(inflation, by = join_by(year)) |>
...
challenge_truth <- d_2022 |>
  group_by(sex) |>
  summarize(
    meanlog = weighted.mean(log(incwage), w = perwt)
  ) |>
  mutate(income = exp(meanlog))

write_csv(challenge_truth, file = "assets/challenge_truth.csv")
